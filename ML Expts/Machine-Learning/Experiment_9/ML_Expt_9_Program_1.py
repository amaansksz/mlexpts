...
scatter = plt.scatter(iris_pca[:, 0], iris_pca[:, 1], c=iris_target,
                      cmap='viridis', edgecolor='k', s=100)
plt.xlabel('Principal Component 1')
plt.ylabel('Principal Component 2')
plt.title('PCA of Iris Dataset - Amaan Shaikh S. 211P052')

# Modified legend creation
for target, target_name in zip(np.unique(iris_target), iris_target_names):
    plt.scatter([], [], c=[plt.cm.viridis(target / 2)], label=target_name,
                edgecolor='k', s=100)
